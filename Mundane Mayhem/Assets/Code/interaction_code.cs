...
public class interaction_code : MonoBehaviour
{
    ...
    public float click_cooldown = 0f;
    ...
    void Update()
    {
        if (dialogue_code.talking) { click_cooldown = .3f; }
        else { click_cooldown -= Time.deltaTime; }
        ...
        switch (act_now)
        {
            ...
            case act_type.quest:
                if (order >= 0)
                { transform.position = Vector3.Lerp(transform.position, new Vector3(order * 2, 1, 1.5f), .2f); }
                else { transform.position = Vector3.Lerp(transform.position, start_pos, .1f); }
                break;

            case act_type.travel:
                if (Vector3.Distance(GameObject.Find("Player").transform.position,transform.position)<3||Vector3.Distance(GameObject.Find("Player").transform.position, transform.position) > 12)
